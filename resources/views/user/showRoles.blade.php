<style>
    .createRole button {
       background: linear-gradient(113.15deg, #3e5871 31.68%, #205CB5 124.3%);
       box-shadow: 5px 5px 5px rgb(57 116 205 / 33%);
       border-radius: 5px;
       color: #fff;
       outline: none;
       border: none;
       ...
       <div class="modal-dialog modal-fullscreen scrollable mw-100 m-2 px-3 py-2" role="document">
           <div class="modal-content">
               <div class="modal-header d-flex justify-content-between title_style">
                   <div>
                       <h5 class="modal-title text-white" id="exampleModalLabel">Add New Role</h5>
                   </div>
                   <div>
   ...
   {{-- Modal End --}}
   <div class="row">
       <div class="createRole d-flex justify-content-end"  style="margin-left: 89%">
           ...
           <tbody>
               @foreach ($roles as $role)
                   <tr>
                       <td>{{ @$role['id'] }}</td>
                       <td>{{ @$role['name'] }}</td>
                       <td>{{ date('d-m-y H:m', strtotime(@$role['created_at'])) }}</td>
                       <td>
                           <button class="edit-button" id="{{ @$role['id'] }}" onclick="updateRole(this.id)"
