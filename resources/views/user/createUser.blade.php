       ...
       {{-- -Tahir Ismail --}}
                ...
                <div class="col-sm-10 col-md-6">
                    ...
                    <select class="form-control" name="role_id" id="role_id">
                        @foreach ($roles as $role)
                            <option value="{{ @$role['id'] }}" name="{{ @$role['name'] }}">{{ @$role['name'] }}
                            </option>
                        @endforeach
                    </select>
                </div>
        ...
        <div class="row">
            <div class="col-12 d-flex justify-content-end">
                @if(@$user['id'])
                <button type="button"
                    style="background: linear-gradient(102.3deg, #88CDFF 41.05%, #4E8DD5 118.58%);
                box-shadow: 5px 5px 5px rgba(105, 108, 255, 0.44);
                border-radius: 10px;outline:none;border:none;color:white;padding:7px 22px;margin:55px 0px"
